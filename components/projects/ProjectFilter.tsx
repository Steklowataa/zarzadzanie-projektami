        ...
        <>
          ...
          <div className="absolute left-0 mt-2 w-56 bg-[#1a1a1a] border border-white/10 rounded-xl shadow-2xl z-40 overflow-hidden backdrop-blur-xl">
            <button
              ...
              className={`w-full text-left px-4 py-3 text-xs cursor-pointer font-bold flex items-center gap-2 hover:bg-white/5 transition-colors ${!showAll ? "text-[#B9FF68]" : "text-gray-400"}`}>
              <Layers size={14} />
              Active Project
            </button>
            <button
              onClick={() => { setShowAll(true); setIsOpen(false); }}
              className={`w-full text-left px-4 py-3 cursor-pointer text-xs font-bold flex items-center gap-2 hover:bg-white/5 transition-colors ${showAll ? "text-[#B9FF68]" : "text-gray-400"}`}>
              <LayoutGrid size={14} />
              Show All Projects
            </button>
          </div>
